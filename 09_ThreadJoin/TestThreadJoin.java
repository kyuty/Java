...
public class TestThreadJoin {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("thread id = " + Thread.currentThread().getId() + "\t t1.start time = " + System.currentTimeMillis());
        t1.start();

        // start second thread after waiting for 2 seconds or if it's dead
        // 等待2s或者t1线程dead，再往下执行，去start t2线程
        try {
            System.out.println("thread id = " + Thread.currentThread().getId() + "\t t1 join time = " + System.currentTimeMillis());
            t1.join(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("thread id = " + Thread.currentThread().getId() + "\t t2.start time = " + System.currentTimeMillis());
        t2.start();
        ...
        System.out.println("thread id = " + Thread.currentThread().getId() + "\t t3.start time = " + System.currentTimeMillis());
        t3.start();
...
class MyRunnable implements Runnable {
    ...
    @Override
    public void run() {
        System.out.println("thread id = " + Thread.currentThread().getId() + "\t Thread started name = " + Thread.currentThread().getName());
        try {
            Thread.sleep(4000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("thread id = " + Thread.currentThread().getId() + "\t Thread ended name = " + Thread.currentThread().getName());
    }
